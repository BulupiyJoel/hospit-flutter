...
import 'package:hospit/controller/route_controller.dart';
...
class _MapHospitalState extends State<MapHospital> {
  final MapController _mapController =
      MapController(); // Contrôleur de la carte FlutterMap
  LatLng?
      _currentLocation; // Stocke la position actuelle de l'utilisateur (latitude et longitude)
  final Location _location =
      Location(); // Instance de l'objet Location pour gérer la localisation

  // Fonction pour initialiser la localisation de l'utilisateur
  Future<void> _initializeLocation() async {
    if (!await _checkRequestPermissions())
      return; // Vérifie et demande les permissions
    final locationData =
        await _location.getLocation(); // Récupère les données de localisation
    setState(() {
      _currentLocation = LatLng(locationData.latitude!,
          locationData.longitude!); // Met à jour l'état avec la position
    });
  }
  ...
  Future<bool> _checkRequestPermissions() async {
    bool serviceEnabled = await _location
        .serviceEnabled(); // Vérifie si le service de localisation est activé
    if (!serviceEnabled) {
      serviceEnabled =
          await _location.requestService(); // Demande l'activation du service
      if (!serviceEnabled)
        return false; // Si l'utilisateur refuse, retourne false
    }

    PermissionStatus permissionGranted =
        await _location.hasPermission(); // Vérifie les permissions
    if (permissionGranted == PermissionStatus.denied) {
      permissionGranted =
          await _location.requestPermission(); // Demande les permissions
  ...
  Future<void> _userCurrentLocation() async {
    if (_currentLocation != null) {
      _mapController.move(
          _currentLocation!, 15.0); // Déplace la carte et ajuste le zoom
    } else {
      showMessage(context,
          "Localisation actuelle non disponible"); // Affiche un message d'erreur
    }
  }

  // Récupération des hôpitaux depuis Firestore
  final Stream<QuerySnapshot> hospitals = firebaseFirestore
      .collection("hospital")
      .snapshots(); // Flux de données des hôpitaux
    ...
    return StreamBuilder(
      ...
      builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
        if (snapshot.hasError) {
          return _buildScaffold(const Text(
              "Erreur de chargement des hôpitaux")); // Gestion des erreurs
        }
              ...
              child: Row(
                ...
                children: [
                  CircularProgressIndicator(
                      color: Colors.lightGreen), // Indicateur de chargement
                  SizedBox(width: 10),
                  Text("Patientez...")
                ],
              ),
        ...
        return Scaffold(
          ...
          body: FlutterMap(
            ...
            options: MapOptions(
              initialCenter: _currentLocation ??
                  const LatLng(-1.2878,
                      29.8666), // Centre initial de la carte (si _currentLocation est null, on utilise une position par défaut)
            ...
            children: [
              TileLayer(
                urlTemplate:
                    "https://tile.openstreetmap.org/{z}/{x}/{y}.png", // URL des tuiles de la carte (OpenStreetMap)
              ),
              ...
              MarkerLayer(
                  markers:
                      hospitalMarkers), // Ajoute les marqueurs des hôpitaux
            ],
          ),
          bottomNavigationBar: BottomNavigationBar(
            ...
            onTap: (index) async {
              await _initializeLocation(); // Update the current location

              if (index == 0) {
                // Handle Departure action
                RouteController routeController = RouteController();
                RouteModel routeModel = RouteModel(
                    lat: _currentLocation!.latitude.toString(),
                    lon: _currentLocation!.longitude.toString(),
                    state:
                        "Departure from ${_currentLocation!.latitude}${_currentLocation!.longitude}");

                String response = await routeController.departure(routeModel);
                showMessage(context, response);
              } else if (index == 1) {
                // Handle Arrival action
                RouteController routeController = RouteController();
                RouteModel? lastRouteModel = await _getLastRoute();

                if (lastRouteModel != null) {
                  RouteModel routeModel = RouteModel(
                      lat: _currentLocation!.latitude.toString(),
                      lon: _currentLocation!.longitude.toString(),
                      state:
                          "Arrival from ${lastRouteModel.lat}${lastRouteModel.lon}");

                  String response = await routeController.arrival(routeModel);
                  showMessage(context, response);
                } else {
                  showMessage(context, "No previous route found");
                }
              }
            },
          ),
  ...
  // Fonction pour récupérer le dernier itinéraire inséré
  Future<RouteModel?> _getLastRoute() async {
    try {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        print("Last route data: $data"); // Debugging line
        return RouteModel(
          lat: data["lat"],
          lon: data["lon"],
          state: data["state"],
      ...
      print("Error getting last route: ${e.toString()}");
    }
    return null;
  }
